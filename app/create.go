...
func (c *cmdCreate) Exec(args []string) error {
	...
	if c.fileName != "" {
		...
		gatewayName = args[0]
		microGateway, err := createMicroGatewayRefModel()
...
func createMicroGatewayRefModel() (types.Microgateway, error) {
			...
			EventHandlers: []types.EventHandler{
				{
					...
					Description: "Handle the user access",
					Params: json.RawMessage(`{
                    				"uri1": "petstore.swagger.io/v2/pet/3"
					}`),
					Reference: "github.com/aambhaik/resources/app.json",
				},
			},
			EventLinks: []types.EventLink{
				{
